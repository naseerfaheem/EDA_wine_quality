...
<br/>
1.2.2 Volatile acidity has almost a similar distribution for both wine types. It seems that red wine has a little higher volatile acitidy (0.53) whereas white volatile acidity level is low (0.28), which makes sense becuase a wine with too more acetic acid could lead to an unpleaseant vinegar taste. <br/>
...
ggplot(wine, aes(residual.sugar, fill=type))+
  coord_cartesian(
    xlim = c(quantile(wine$residual.sugar, .001), quantile(wine$residual.sugar, .99))
  )+
...
ggplot(wine, aes(chlorides, fill=type))+
  coord_cartesian(
    xlim = c(quantile(wine$chlorides, .01), quantile(wine$chlorides, .99))
  )+
...
ggplot(wine, aes(free.sulfur.dioxide, fill=type))+
  coord_cartesian(
    xlim = c(quantile(wine$free.sulfur.dioxide, .01), quantile(wine$free.sulfur.dioxide, .999))
  )+
...
White wine also has a very high concentration of totla sulfur dioxide compared to the red wine. 
...
ggplot(wine, aes(density, fill=type))+
  coord_cartesian(
    xlim = c(quantile(wine$density, 0.001), quantile(wine$density, 0.999))
  )+
...
ggplot(wine, aes(sulphates, fill=type))+
  coord_cartesian(
    xlim = c(quantile(wine$sulphates, 0.001), quantile(wine$sulphates, 0.999))
  )+
  geom_histogram(binwidth = 0.1)+
...
ggplot(aes(alcohol, residual.sugar), data = wine)+
  geom_point(aes(color=factor(quality)), alpha = 0.20, position = 'jitter')+
  geom_smooth(method = 'lm')+
  scale_fill_brewer(type = "div", aesthetics = "RdYlGn")+
...
ggplot(aes(x=residual.sugar), data = wine)+
  coord_cartesian(
    xlim = c(quantile(wine$residual.sugar, 0.001), quantile(wine$residual.sugar, 0.99))
  )+
  geom_density(aes(fill=type))+
  facet_wrap(~quality)+
